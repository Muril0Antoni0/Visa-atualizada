document.addEventListener('DOMContentLoaded', () => {
    ...
    async function inicializarProcessos() {
        ...
        const processosSalvos = salvos ? JSON.parse(salvos) : [];
        ...
        try {
            ...
            if (response.ok) {
                ...
                const apiProcessos = data.processos || [];
                // Mantém os processos criados no portal em primeiro lugar
                processos = processosSalvos.length > 0 ? [...processosSalvos, ...apiProcessos] : apiProcessos;
            } else {
                processos = processosSalvos.length > 0 ? processosSalvos : defaultProcessos;
            }
        } catch (error) {
